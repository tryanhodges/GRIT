...
function initializeEventListeners() {
    ...
    document.body.addEventListener('click', (e) => {
        ...
        if (target.matches('.delete-site-btn')) {
            deleteSite(target.dataset.siteId, target.dataset.siteName);
        }
        if (target.matches('.receive-po-btn')) {
        ...
        if (target.matches('.remove-exclusion-btn')) {
            removeExclusionKeyword(target.dataset.keyword);
        }
        if (target.closest('.rack-title')) {
             appState.selectedRackId = parseInt(target.closest('.rack-title').dataset.rackId);
             toggleView();
        }
        if (target.closest('[data-action="select-rack"]')) {
        ...
        if (target.matches('[data-action="deny-user"]')) {
        ...
        if (target.matches('[data-action="remove-cushion-level"]')) {
    ...
    document.body.addEventListener('change', (e) => {
        const target = e.target;
        if (target.matches('[data-action="update-role"]')) {
            updateUserRole(target.dataset.uid, target.value);
        }
        if (target.matches('.model-assignment-select')) {
