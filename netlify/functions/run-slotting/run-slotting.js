...
if (!admin.apps.length) {
  try {
    ...
    // If initialization fails, log a clear error. This is a common source of crashes.
    console.error("FATAL_ERROR: Firebase Admin SDK initialization failed.", e);
    // We don't proceed if Firebase isn't initialized.
  }
}
  ...
  return new Promise((resolve) => {
    ...
    corsHandler(event, context, () => {
      (async () => {
        ...
        if (!admin.apps.length) {
            console.error("CRASH_POINT: Firebase Admin not initialized. Check environment variables.");
            return callback(500, { error: "Server configuration error. Firebase Admin SDK not initialized." });
        }
